...
checkEvent5 = function(data){
  ...
  condition3 = (ceiling(which(grepl('A', data)) /5) == 1)!=(ceiling(which(grepl('Q', data)) /5) == 1)
...
checkEvent6 = function(data){
  condition1=(unique(ceiling(which(grepl('A', data)) /5)) == unique(ceiling(which(grepl('Q', data)) /5)))
  print(condition1)
  if (sum(grepl("FALSE",condition1))>=1){
    condition2=FALSE
  }
  else
    condition2=TRUE
...
simulatedData_reduced = simulatedData[, apply(simulatedData, 2, function(data) {
  return((sum(grepl('A', data))==2) & (sum(grepl('Q',data))==2))
})]
...
simulatedData_reduced = simulatedData[, apply(simulatedData, 2, function(data){return((sum(grepl('A', data)) == 2) & sum(grepl('Q', data)) == 2 & length(unique(ceiling(which(grepl('A', data)) /5))) == 1 & length(unique(ceiling(which(grepl('A', data)) /5))) == 1)})]
mean(simulatedData_reduced,2,checkEvent6)
```
